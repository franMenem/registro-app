...
import { formatCurrency, formatDate, parseDateFromDB } from '@/utils/format';
  ...
  const renderCell = (column: TableColumn, row: ControlPendiente) => {
    switch (column.key) {
      ...
      case 'fecha_pago_programada':
        const hoy = new Date();
        const fechaPago = parseDateFromDB(row.fecha_pago_programada);
          ...
          <div className="space-y-3">
            {alertasLoading ? (
              ...
              alertasPagos.map((control) => {
                const fechaPago = parseDateFromDB(control.fecha_pago_programada);
                const hoy = new Date();
                const diff = Math.ceil((fechaPago.getTime() - hoy.getTime()) / (1000 * 60 * 60 * 24));
                const isVencido = diff < 0;
